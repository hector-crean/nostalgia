...
use nostalgia_api::services::s3::S3Bucket;
use nostalgia_api::{errors, AppState};
...
async fn main() -> errors::Result<()> {
    ...
    let router = AppState::new(pool, bucket, Arc::new(Mutex::new(random)), tx)
        .router()
        .await?;
...
mod tests {
    ...
    use nostalgia_models::user::User;

    use serde_json::json;
    use std::net::SocketAddr;
    use nostalgia_models::user::{CreateUser, CreateUserResponse};
    use nostalgia_api::services::s3::S3Bucket;
